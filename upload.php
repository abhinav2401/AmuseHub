...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    $repo='assets/img/'.$_POST["name1"].'jpg';
    $email = split_session()[1];
	$target_dir = 'repo/'.$_POST["folder"].'/';
	...
	$name = $target_dir .$email.';'. $_POST["name1"] . '.' .$imageFileType;
  $name2 = $email.';'. $_POST["name1"] . '.' .$imageFileType;
  $itemname = $email.';'. $_POST["name1"] . '.' .$imageFileType;
	$extensions = array("jpg", "jpeg", "png" , "gif", "webp", "webm", "svg", "weba", "mp4", "mp3" , "mp2", "mpg", "opus", "mkv", "xls", "xlsx", "doc", "docx", "rtf", "odt", "pdf", "ppt", "pptx", "txt");
	...
	// if everything is ok, try to upload file
	} else
	 {
	  ...
	  if (move_uploaded_file($_FILES["filetoupload"]["tmp_name"], $name)) {
        ...
        $fqf=$folder_root .$name;
        $com2 = "UPDATE item SET files = concat(name, '{$itemname}') WHERE EMAILID = '{$email}'";
        $db->query($com2);
        $com="UPDATE {$table_userdata} SET files = concat(files, '{$fqf}', ';') WHERE EMAILID = '{$email}'";
        $db->query($com);
          $com4="INSERT INTO item (name) VALUES ('{$name2}')";
          $result2=$db->query($com4);
...
<body>
    ...
    <form action="" class="form-signin" method="post"
        ...
        <h2 class="h3 mb-3 font-weight-normal text-center" style="font-size:40px; color:white;"> Upload Files</h2>
			...
			<input type="text" class="form-control" name="name1" id="name1" placeholder="File name after upload (without extension)"required>
            </br>
      ...
      <?php
           ...
           $resource_url=$repo_root;
		   $dirs = array_diff(scandir($resource_url), array('..', '.'));
